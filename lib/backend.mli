(** Storage backends for Irmin.
    ...
    Backends are records of functions, NOT functors. This makes them
    composable and easy to create without functor application. *)
...
type 'hash t = {
  ...
  exists : 'hash -> bool;
  (** [exists hash] checks if an object exists. *)
  ...
  (** [test_and_set_ref name ~test ~set] atomically updates a reference
      if its current value matches [test]. *)
  ...
  flush : unit -> unit;
  (** [flush ()] ensures all writes are persisted. *)
  close : unit -> unit;
...
val cached : 'h t -> 'h t
(** [cached backend] wraps a backend with an LRU cache.
    Reads are served from cache when possible. *)

val readonly : 'h t -> 'h t
(** [readonly backend] makes a backend read-only.
    Write operations raise [Invalid_argument]. *)

val layered : upper:'h t -> lower:'h t -> 'h t
(** [layered ~upper ~lower] creates a layered backend.
    Reads check upper first, then lower.
    Writes go to upper only.
    Used for garbage collection (upper=live, lower=frozen). *)
