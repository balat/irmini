...
module Git : sig
  ...
  module Store : module type of Store.Git
  module Subtree = Subtree.Git
  module Proof = Proof.Git

  val import :
    sw:Eio.Switch.t -> fs:Eio.Fs.dir_ty Eio.Path.t -> git_dir:Fpath.t -> Store.t
  (** Open a bare .git directory as a store. *)

  val init :
    sw:Eio.Switch.t -> fs:Eio.Fs.dir_ty Eio.Path.t -> path:Fpath.t -> Store.t
  (** Initialize a new Git repository and return a store. *)
end
...
module Mst : sig
  ...
  module Store : module type of Store.Mst
